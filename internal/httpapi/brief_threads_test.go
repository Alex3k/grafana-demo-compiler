...
func TestApplyBriefTopic(t *testing.T) {
	...
	if !applyBriefTopic(&content, "Telemetry: Structured logs", "Structured JSON firmware events") {
	...
	if !applyBriefTopic(&content, "Scenario", "Camera config outage") || content.Scenario.Status != "confirmed" || content.Scenario.Value != "Camera config outage" {
